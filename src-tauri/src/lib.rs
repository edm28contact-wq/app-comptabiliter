    ...
    let result = connection.query_row(
        ...
        |row| {
            ...
            Ok(AccountingAssignment {
                ...
                confidence: (75 + (use_count.saturating_sub(1) * 5)).min(95) as i32,
    ...
    connection.execute(
        "UPDATE invoices
         SET extracted_text=?2, extraction_status=?3, extraction_error=NULL,
             text_length=?4, parsed_json=?5, updated_at=CURRENT_TIMESTAMP
         WHERE path=?1",
    ...
    match pdf_extract::extract_text(path) {
        Ok(text) => {
            ...
            if length >= 40 {
                ...
                connection.execute(
                    "UPDATE invoices
                     SET extracted_text=?2, extraction_status='ocr_requis', text_length=?3,
                         updated_at=CURRENT_TIMESTAMP WHERE path=?1",
        ...
        Err(error) => {
            connection.execute(
                "UPDATE invoices
                 SET extraction_status='ocr_requis', extraction_error=?2, text_length=0,
                     updated_at=CURRENT_TIMESTAMP WHERE path=?1",
...
fn validate_invoice(
    ...
    connection.execute(
        "UPDATE invoices
         SET validated_json=?2, validated_accounting_json=?3, validated_at=CURRENT_TIMESTAMP,
             status='validee', updated_at=CURRENT_TIMESTAMP
         WHERE path=?1",
        params![path, invoice_json, accounting_json],
    ).map_err(|error| error.to_string())?;
