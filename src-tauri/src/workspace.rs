use crate::{charlemagne::PreparedCharlemagneEntry, windows_ocr};
...
pub fn list_journal_entries(app: AppHandle) -> Result<Vec<JournalEntryRow>, String> {
    ...
    let mut statement = connection
        .prepare(
            "SELECT prepared_charlemagne_json FROM invoices WHERE prepared_charlemagne_json IS NOT NULL AND status IN ('validee','classee','archive_source_presente') ORDER BY validated_at ASC",
        )
        .map_err(|error| error.to_string())?;
