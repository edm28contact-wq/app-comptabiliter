...
fn status_for_mode(mode: &str) -> CharlemagneConnectorStatus {
    match mode {
        MODE_API_V2 => CharlemagneConnectorStatus {
            mode: MODE_API_V2.to_string(),
            version_label: "Version 2".to_string(),
            transport_label: "API officielle / partenaire".to_string(),
            live_ready: false,
            preparation_ready: true,
            blocked_reason: Some(
                "Accès API/SDK Aplim non configuré. Le mode peut être sélectionné sans déclencher d'envoi."
                    .to_string(),
            ),
        ...
        _ => CharlemagneConnectorStatus {
            ...
            transport_label: "Fichier d'import Charlemagne".to_string(),
            live_ready: false,
            preparation_ready: true,
            blocked_reason: Some(
                "Le format d'import officiel Aplim doit être configuré avant de générer un fichier destiné à Charlemagne."
                    .to_string(),
            ),
            switch_available: true,
        },
...
#[cfg(test)]
mod tests {
    use super::{normalize_mode, status_for_mode, MODE_API_V2, MODE_IMPORT_V1};
    ...
    #[test]
    fn api_mode_is_switchable_without_enabling_live_writes() {
        let status = status_for_mode(MODE_API_V2);
        assert_eq!(status.mode, MODE_API_V2);
        assert!(status.switch_available);
        assert!(!status.live_ready);
    }
